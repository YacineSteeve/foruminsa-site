...
export default async function CompaniesPage({ searchParams }: CompaniesPageProps) {
    ...
    const filters: CompaniesFilters = {
        ...
        country: (!Array.isArray(filtersParams.country) && COUNTRY_CODES.includes(filtersParams.country as typeof COUNTRY_CODES[number]))
            ? filtersParams.country as typeof COUNTRY_CODES[number]
            : undefined,
        speciality: (!Array.isArray(filtersParams.speciality) && SPECIALITIES.includes(filtersParams.speciality as typeof SPECIALITIES[number]))
            ? filtersParams.speciality as typeof SPECIALITIES[number]
            : undefined,
        studyLevel: (!Array.isArray(filtersParams.studyLevel) && STUDY_LEVELS.includes(filtersParams.studyLevel as typeof STUDY_LEVELS[number]))
        ...
        <div className="w-full">
            ...
            <div className="flex flex-col items-center gap-y-16 w-full px-4 md:px-10 lg:px-20 xl:px-40 2xl:px-60 py-40 max-md:pt-60">
                <div className="flex flex-col items-center gap-y-4 text-center">
                    <h1>{t('title')}</h1>
                    <p className="text-xl w-full sm:w-5/6 md:w-4/5 lg:w-3/4 xl:w-2/3 2xl:w-1/2">{t('description')}</p>
                </div>
